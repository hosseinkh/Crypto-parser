# utiles/trending.py
# -----------------------------------------------------------
# Trend scanner with "reason strings" for why a symbol is added.
# -----------------------------------------------------------
...
class TrendScanParams:
    ...
    min_volz_24h: float = 0.8         # 24h volume anomaly
...
def _compute_24h_vol_z(df: pd.DataFrame, bars_24h: int, window: int = 20) -> float:
    """Sum last 24h of volumes and compute z-score of that sum."""
...
def scan_trending(
    ...
    for sym in universe:
        try:
            ...
            pct4h = _pct_change_over_n(df, n=params.bars_for_4h())
            volz24 = _compute_24h_vol_z(df, bars_24h=params.bars_for_24h(), window=20)
            ...
            rows.append({
                ...
                "vol_z24h": volz24,
                "rsi14_15m": rsi_15m,
                "sentiment_score": 0.0,  # TODO: hook sentiment API
